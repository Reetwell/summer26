...
struct ExerciseDetailView: View {
    ...
    private var muscleLegend: some View {
        VStack(alignment: .leading, spacing: Spacing.sm) {
            legendRow(color: .green500, label: "Primary", muscles: exercise.primaryMuscles)
            if !exercise.secondaryMuscles.isEmpty {
                legendRow(color: Color.green500.opacity(0.38), label: "Secondary", muscles: exercise.secondaryMuscles)
            }
        }
    ...
    @ViewBuilder
    private func legendRow(color: Color, label: String, muscles: [Exercise.Muscle]) -> some View {
...
// MARK: - Anatomical muscle map (real polygon data, ported from react-body-highlighter, MIT)
...
struct MuscleMapView: View {
    ...
    // True aspect of the source viewBox (100 × 220 coordinate space).
    private let mapWidth: CGFloat = 106
    private var mapHeight: CGFloat { mapWidth * BodyMuscleData.coordHeight / 100 }

    var body: some View {
        let allPolys = BodyMuscleData.regions(view).flatMap { $0.polys }

        VStack(spacing: Spacing.sm) {
            ZStack {
                // Base anatomy — every region faint so the whole body reads as a figure
                MusclePolyShape(polys: allPolys)
                    .fill(Color.secondary.opacity(0.12))
                MusclePolyShape(polys: allPolys)
                    .stroke(Color.secondary.opacity(0.22), lineWidth: 0.6)
                // Secondary movers
                MusclePolyShape(polys: highlightPolys(secondary))
                    .fill(Color.green500.opacity(0.38))
                // Primary movers pop on top
                MusclePolyShape(polys: highlightPolys(primary))
                    .fill(Color.green500.opacity(0.92))
                MusclePolyShape(polys: highlightPolys(primary))
                    .stroke(Color.green500, lineWidth: 0.6)
            }
    ...
    /// Polygons for the regions worked by the given muscles, in this view.
    private func highlightPolys(_ muscles: [Exercise.Muscle]) -> [String] {
        let slugs = Set(muscles.flatMap { BodyMuscleData.slugs(for: $0, view: view) })
        guard !slugs.isEmpty else { return [] }
        return BodyMuscleData.regions(view)
...
/// Builds a closed path from one or more "x y x y …" polygon strings,
/// scaling the 100 × 220 source space into the given rect.
struct MusclePolyShape: Shape {
    let polys: [String]

    func path(in rect: CGRect) -> Path {
        var p = Path()
        let sx = rect.width / 100
        let sy = rect.height / BodyMuscleData.coordHeight
        for poly in polys {
            let nums = poly.split(separator: " ").compactMap { Double($0) }
            var i = 0
            var started = false
            while i + 1 < nums.count {
                let pt = CGPoint(x: CGFloat(nums[i]) * sx, y: CGFloat(nums[i + 1]) * sy)
...
// MARK: - Body polygon data (viewBox 100 × 220, from react-body-highlighter, MIT)

enum BodyMuscleData {
    static let coordHeight: CGFloat = 220

    static func regions(_ view: BodyView) -> [(slug: String, polys: [String])] {
        view == .front ? anterior : posterior
    }

    /// Map our `Exercise.Muscle` cases to the highlighter's region slugs, per view.
    static func slugs(for muscle: Exercise.Muscle, view: BodyView) -> [String] {
        switch (muscle, view) {
        case (.chest, .front):      return ["chest"]
        case (.shoulders, .front):  return ["front-deltoids"]
        case (.shoulders, .back):   return ["back-deltoids"]
        case (.biceps, .front):     return ["biceps"]
        case (.triceps, _):         return ["triceps"]
        case (.forearms, _):        return ["forearm"]
        case (.core, .front):       return ["abs", "obliques"]
        case (.quads, .front):      return ["quadriceps"]
        case (.traps, .back):       return ["trapezius"]
        case (.lats, .back):        return ["upper-back"]
        case (.back, .back):        return ["upper-back", "lower-back"]
        case (.glutes, .back):      return ["gluteal"]
        case (.hamstrings, .back):  return ["hamstring"]
        case (.calves, .back):      return ["calves", "left-soleus", "right-soleus"]
        default:                    return []
        }
    }

    static let anterior: [(slug: String, polys: [String])] = [
        ("chest", [
    ...
    static let posterior: [(slug: String, polys: [String])] = [
        ("head", [
